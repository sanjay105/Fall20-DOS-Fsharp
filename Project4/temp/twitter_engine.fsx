...
let TweetHandler (mailbox:Actor<_>) =
    ...
    let rec loop() = actor{
        ...
        match msg with
        | AddTweet(username,tweetid,tweet,req) ->   if cnt = 0 then
                                                        timer.Start()
                                                    let userid = getUserID username
                                                    // printfn "%s username and userid %s" username userid
                                                    ...
                                                    cnt <- cnt + 1
                                                    // printfn "Processed %i Tweets in %i from owner %s" cnt timer.ElapsedMilliseconds username
                                                    if cnt % 1000 = 0 then
                                                        printfn "Processed %i tweets in %i milliseconds" cnt timer.ElapsedMilliseconds
                                                    // req <! "ACK|Tweet|"
