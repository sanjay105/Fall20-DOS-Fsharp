...
// let mutable exhaustedPnode = Map.empty
                                                ...
                                                if(topology="line") then
                                                    ...
                                                    for i in 0 .. nodecount-1 do
                                                        ...
                                                        if topology = "imp2D" then 
                                                            ...
                                                            //     next <- rnd.Next()
                                                            nlist <- nlist @ [nodelist.Item(nodecount-1-i)]
                                                        //nodelist.Item(i)<!AddTopology(topology,dispatcherref,nlist)
                                                        if algo = "gossip" then
                                                            nodelist.Item(i)<!AddTopology(topology,dispatcherref,nlist)  
                                                        else
    ...
    let rec loop() =actor{
        ...
        | Receive(rs,rw) ->  
                            if not exhausted then
                                ...
                                if ecnt = 3 then 
                                    exhausted <- true
                                    // exhaustedPnode <- exhaustedPnode.Add(id,"a")
                                    dispatcherref<!NodeExhausted
                                    if topology = "full" then
                                        for i in 0 .. nodecount-1 do 
                                            if i <> id &&  not (dead.ContainsKey(i%nodecount)) then
                                                Nodelist.Item(i)<!ExhaustedP
                                                Nodelist.Item(i)<!Receive(ns/2.0,nw/2.0)
                                                
                                    else
                                        for i in 0 .. neigbhours.Length-1 do
                                            if not (dead.ContainsKey(neigbhours.Item(i).Path.Name|>int)) then
                                                neigbhours.Item(i)<!ExhaustedP
                                                neigbhours.Item(i)<!Receive(ns/2.0,nw/2.0)
        ...
        | Send ->
                    if not exhausted then
                        ...
                        w <- w/2.0
                        if topology = "full" then
                            while next%nodecount=id || dead.ContainsKey(next%nodecount) do
                                next<-rnd.Next()
                            Nodelist.Item(next%nodecount)<!Receive(s,w)
                        else
                            
                            while dead.ContainsKey(neigbhours.Item(next%neigbhours.Length).Path.Name|>int) do
                                next <- rnd.Next()
                            neigbhours.Item(next%neigbhours.Length)<!Receive(s,w)

        | ExhaustedP -> dead <- dead.Add(mailbox.Sender().Path.Name|>int,"true")
